...
import static com.nuvyra.launcher.bridge.ZLBridgeStatesKt.CURSOR_DISABLED;
import static com.nuvyra.launcher.bridge.ZLBridgeStatesKt.CURSOR_ENABLED;
...
public class CallbackBridge {
    ...
    public static boolean notifyLauncher(int type, int... action) {
        if (action == null || action.length == 0) {
            LoggerBridge.append("NuvyraLauncher: SDL notification has no action");
            return false;
        }
        switch (type) {
            case NOTIF_TYPE_SDL:
                if (action[0] == ACTION_INIT_LAUNCHER_INTEGRATION) {
                    ...
                    try {
                        LoggerBridge.append("NuvyraLauncher: loading SDL3");
                        System.loadLibrary("SDL3");
                        LoggerBridge.append("NuvyraLauncher: loading SDL2");
                        System.loadLibrary("SDL2");
                        LoggerBridge.append("NuvyraLauncher: setting up SDL JNI");
                        SdlBridge.setupJNI();
                        LoggerBridge.append("NuvyraLauncher: binding SDL surface");
                        ...
                        LoggerBridge.append("NuvyraLauncher: SDL support enabled!");
                        return true;
                    } catch (Throwable e) {
                        ...
                        LoggerBridge.append("NuvyraLauncher: SDL launcher integration is unavailable:\n" + trace);
                    }
                }
    ...
    private static void onDirectInputEnable() {
        LoggerBridge.append("NuvyraLauncher: Direct gamepad input enabled");
